...
export function cotContactEvidence(output, result) {
    ...
    const facebookPhoneUrl = facebookPhone ? result.facebookEvidenceUrl : null;
    ...
    const googleAddress = result.addressSource === 'google-official-website-structured'
        && result.addressIdentityStatus === 'matched' && result.addressVerified === true;
    const facebookAddressUrl = matched ? result.facebookEvidenceUrl : null;
    ...
    const addressSource = googleAddress ? result.addressSource : 'facebook-page-contact';
    return {
        evidence: {
            ...output.evidence,
