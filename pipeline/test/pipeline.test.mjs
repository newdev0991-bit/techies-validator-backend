...
test('a transient preflight connection failure is retried instead of halting the pipeline',async t=>{
  // Preflight runs BEFORE any charge or Actor start, so retrying it cannot double-spend.
  // Halting instead is what stopped lead collection for hours on a single blip, with no
  // recovery path for the code it set.
  const f=fixture(t);
  f.p.preflight=async()=>{const e=new Error('PROVIDER_CONNECTION_UNCERTAIN');e.code='PROVIDER_CONNECTION_UNCERTAIN';throw e;};

  const first=await f.runner.tick();
  assert.equal(first.status,'preflight_retry');
  assert.equal(f.s.get('halted'),null);
  assert.equal(f.calls.start,0);

  // Once the validator recovers, the next eligible tick proceeds normally.
  f.p.preflight=async()=>{f.calls.preflight++;};
  f.advance(600000);
  assert.equal((await f.runner.tick()).status,'search_started');
});

test('a validator that stays unreachable eventually halts rather than retrying forever',async t=>{
  const f=fixture(t);
  f.p.preflight=async()=>{const e=new Error('PROVIDER_CONNECTION_UNCERTAIN');e.code='PROVIDER_CONNECTION_UNCERTAIN';throw e;};
  let last;
  for(let i=0;i<6;i++){last=await f.runner.tick();f.advance(600000);}
  assert.equal(last.status,'halted');
  assert.equal(f.s.get('halted').code,'PREFLIGHT_UNAVAILABLE');
  assert.equal(f.calls.start,0);
});

test('a non-transient preflight failure still halts immediately',async t=>{
  const f=fixture(t);
  f.p.preflight=async()=>{const e=new Error('VALIDATOR_NOT_READY');e.code='VALIDATOR_NOT_READY';throw e;};
  const r=await f.runner.tick();
  assert.equal(r.status,'halted');
  assert.equal(r.code,'VALIDATOR_NOT_READY');
});
